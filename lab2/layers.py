...
from im2col_cython import col2im_cython, im2col_cython # type: ignore
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    """
    exps = np.exp(x)
    sumexps = np.reshape(np.sum(exps, axis=1), (-1, 1))
    probs = exps / sumexps
    logprobs = np.log(probs)
    return - (1/x.shape[0]) * np.sum(y * logprobs)
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    return 0.5 * self.weight_decay * np.linalg.norm(self.weights) ** 2
